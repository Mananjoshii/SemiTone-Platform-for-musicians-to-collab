...
app.get('/band/:id', async (req, res) => {
    ...
    try {
        ...
        console.log(membersResult.rows);  // Check if members data is correct
        
        res.render('band_profile', {
            band,
            members: membersResult.rows,  // Ensure `members` is passed here
...
app.get('/bandProfile', async (req, res) => {
    ...
    try {
        ...
        res.render('band_profile', { band: result.rows }); // Render the band_profile.ejs template
    } catch (error) {
        console.error(error);
